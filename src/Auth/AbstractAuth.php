...
/*
 * @copyright   2017 Reizee\Api\ReizeeApi Contributors. All rights reserved
...
abstract class AbstractAuth implements AuthInterface
{
    ...
    {
        return ($this->_debug && !empty(session('oauth.debug'))) ? session('oauth.debug') : [];
    }
    ...
    public function makeRequest($url, array $parameters = [], $method = 'GET', array $settings = [])
    {
        ...
        if ($this->_debug) {
            session()->put('oauth.debug.info', $response->getInfo());
            session()->put('oauth.debug.returnedHeaders', $response->getHeaders());
            session()->put('oauth.debug.returnedBody', $response->getBody());
        }
    ...
    {
        if ($this->_debug) {
            session()->push('oauth.debug.flow.' . date('m-d H:i:s') , $message);
        }
    }
