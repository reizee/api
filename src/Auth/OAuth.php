...
use Reizee\Api\Exception\AuthorizationRequiredException;
use Reizee\Api\Exception\IncorrectParametersReturnedException;
...
class OAuth extends AbstractAuth
{
    ...
    /**
     * @param string $baseUrl            URL of the Reizee\Api\ReizeeApi instance
    ...
    public function validateAccessToken($redirect = true)
    {
        ...
        if (0 == strlen($this->_access_token)) {
            ...
            if ($this->isOauth1()) {
                ...
                if ($_GET['oauth_token'] != session('oauth.token')) {
                    session()->forget(['oauth.token', 'oauth.token_secret']);
                    return false;
                }
            ...
            if ($this->_debug) {
                session()->put('oauth.debug.received_state', $_GET['state']);
            }

            //Request an access token
            if ($_GET['state'] != session('oauth.state')) {
    ...
    protected function authorize(array $scope = [], $scope_separator = ',', $attach = null)
    {
        ...
        if ($this->isOauth1()) {
            ...
            if ($this->_debug) {
                session()->put('oauth.debug.generated_state', $state);
            }
    ...
    protected function getQueryParameters($isPost, $parameters)
    {
        ...
        if (isset($parameters['file'])) {
            // Reizee\Api\ReizeeApi's OAuth2 server does not recognize multipart forms so we have to append the access token as part of the URL
    ...
    protected function prepareRequest($url, array $headers, array $parameters, $method, array $settings)
    {
        ...
        if ($this->isOauth1()) {
            ...
            if ($includeVerifier && isset($_GET['oauth_verifier'])) {
                ...
                if ($this->_debug) {
                    session()->put('oauth.debug.oauth_verifier', $_GET['oauth_verifier']);
                }
            }
            ...
            if ($this->_debug) {
                session()->put('oauth.debug.basestring', $base_info);
                session()->put('oauth.debug.headers', $headers);
            }
        } else {
            //OAuth 2.0
    ...
    protected function requestAccessToken($method = 'POST', array $params = [], $responseType = 'flat')
    {
        ...
        if (is_array($params)) {
            if ($this->isOauth1()) {
                ...
                if (isset($params['oauth_token']) && isset($params['oauth_token_secret'])) {
                    ...
                    if ($this->_debug) {
                        session()->put('oauth.debug.tokens.oauth_token', $params['oauth_token']);
                        session()->put('oauth.debug.tokens.oauth_token_secret', $params['oauth_token_secret']);
                    }
                ...
                if (isset($params['access_token']) && isset($params['expires_in'])) {
                    ...
                    if ($this->_debug) {
                        session()->put('oauth.debug.tokens.access_token', $params['access_token']);
                        session()->put('oauth.debug.tokens.expires_in', $params['expires_in']);
                        session()->put('oauth.debug.tokens.token_type', $params['token_type']);
                        session()->put('oauth.debug.tokens.refresh_token', $params['refresh_token']);
                    }
        ...
        if ($this->_debug) {
            session()->put('oauth.debug.response', $params);
        }
    ...
    protected function requestToken($responseType = 'flat')
    {
        ...
        if (is_array($params) && isset($params['oauth_token']) && isset($params['oauth_token_secret'])) {
            ...
            session()->put('oauth.token', $params['oauth_token']);
            session()->put('oauth.token_secret', $params['oauth_token_secret']);

            if ($this->_debug) {
                session()->put('oauth.debug.token', $params['oauth_token']);
                session()->put('oauth.debug.token_secret', $params['oauth_token_secret']);
            }
        } else {
    ...
    private function getCompositeKey()
    {
        ...
        } elseif (session('oauth.token_secret')) {
